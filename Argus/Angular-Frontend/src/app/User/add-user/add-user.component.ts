...
export class AddUserComponent implements OnInit {
  ...
  save() {
    ...
    if ((usernameInp.value !== '') && (emailInp.value !== '') && (nameInp.value !== '') && (surnameInp.value !== '') &&
      (passwordInp.value !== ''))
    {
      const imgsrc = this.imgTob64();

      if (imgsrc != null)
      {
        this.newUser = new User();
        this.newUser.profilePhoto = imgsrc;
  ...
  imgTob64() {
    if (this.useDefaultImg) {
      return document.getElementById('profilePicDisplay').getAttribute('src');
    } else {
      return document.getElementById('submitPhoto').getAttribute('src');
    }
  }
